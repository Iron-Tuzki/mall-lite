...
| `cancel_type` | `tinyint unsigned` | 否 | `NULL` | 取消类型：10 用户主动取消，20 超时自动取消 |
| `cancel_reason` | `varchar(100)` | 否 | `NULL` | 取消原因 |
...
| `pending_order_id` | `bigint unsigned` | 否 | 生成列 | 待支付订单 ID，仅当支付流水为待支付且未删除时等于 `order_id` |
...
| `uk_pending_order_channel` | `pending_order_id, pay_channel` | 唯一索引 | 同一订单同一渠道只能有一条待支付流水 |
