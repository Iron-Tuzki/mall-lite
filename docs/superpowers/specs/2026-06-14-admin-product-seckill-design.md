...
后台商品写操作需要清理商品详情缓存和热门商品详情缓存，避免前台看到旧数据。触发范围包括新增商品、编辑商品、删除商品、上架和下架；缓存失效粒度按 `productId` 删除普通详情缓存和热门详情缓存。

缓存失效采用异步延迟二删：

1. 商品、SKU、库存写入必须先在数据库事务内完成。
2. 事务提交成功后，通过 `afterCommit` 回调立即删除一次 Redis 商品详情缓存，避免事务未提交时前台回源读到旧数据并重新写入缓存。
3. 立即删除后不在后台请求线程中 `sleep`，而是交给独立 `TaskScheduler` 调度延迟任务。
4. 延迟任务默认在 500ms 后再次删除同一 `productId` 的普通详情缓存和热门详情缓存，用于覆盖“第一次删除后并发读请求回源旧数据并写回缓存”的窗口。
5. 延迟时间保留配置项，例如 `mall.product.cache.delayed-delete-millis`，后续可按压测和线上读写延迟调整。
6. 缓存删除失败不回滚商品写事务，但需要记录 warn 日志；下一次商品更新、缓存 TTL 到期或人工运维仍可恢复一致性。
...
3. 删除商品后前台商品查询不可见。
4. 商品编码和 SKU 编码唯一校验。
5. 商品写操作提交后会立即删除详情缓存，并调度延迟二删任务。
6. 秒杀活动时间校验。
7. 秒杀商品不能重复选择。
8. 秒杀价不能高于 SKU 原价。
9. 秒杀商品删除后活动详情不再返回。
10. 手动预热接口仍能把活动库存写入 Redis。
